...
int main(int argc, char* argv[]) 
{
  ...
  std::cout << "number of off-diagnal elements: " << num_edge << std::endl;
  ...
  if ( argc >= 3 ) {
    ...
    if ( input.fail() ) {
      std::cout << argv[3] << " is failed to open!. " << std::endl;
      return -1;
    }
    int comp;
    ...
    for ( i=0; i<n; i++ ) {
      ...
      if ( comp != inverse_perm[i]+1 ) {
        std::cout << "at i= " << i << ": " << comp
             << " ***is NOT EQUAL to*** " << inverse_perm[i]+1 << std::endl;
        is_correct = false;
      }      
    }
    for ( i=0; i<n; i++ ) {
      ...
      if ( comp != perm[i]+1 ) {
        std::cout << "at i= " << i << ": " << comp
             << " ***is NOT EQUAL to*** " << perm[i]+1 << std::endl;
        is_correct = false;
      }
    }
    if ( is_correct ) 
      std::cout << "Permutation and inverse permutation are correct. "<< std::endl;
    else 
      std::cout << "WARNING -- Permutation or inverse permutation is not the "
           << "same ones generated by Liu's " << std::endl;
