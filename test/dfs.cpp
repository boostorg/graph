...
public:
  ...
  void initialize_vertex(Vertex u, Graph&) {
    BOOST_TEST_EQ( boost::get(m_color, u), Color::white() );
  }
  template <class Vertex, class Graph>
  ...
  void discover_vertex(Vertex u, Graph&) {
    ...
    BOOST_TEST_EQ( get(m_color, u), Color::gray() );
    BOOST_TEST_EQ( get(m_color, get(m_parent, u)), Color::gray() );
  ...
  void examine_edge(Edge e, Graph& g) {
    using namespace boost;
    BOOST_TEST_EQ( get(m_color, source(e, g)), Color::gray() );
  }
  template <class Edge, class Graph>
  void tree_edge(Edge e, Graph& g) {
    using namespace boost;
    BOOST_TEST_EQ( get(m_color, target(e, g)), Color::white() );
  ...
  void back_edge(Edge e, Graph& g) {
    using namespace boost;
    BOOST_TEST_EQ( get(m_color, target(e, g)), Color::gray() );
  }
  template <class Edge, class Graph>
  void forward_or_cross_edge(Edge e, Graph& g) {
    using namespace boost;
    BOOST_TEST_EQ( get(m_color, target(e, g)), Color::black() );
  }
  template <class Edge, class Graph>
  ...
  void finish_vertex(Vertex u, Graph&) {
    using namespace boost;
    BOOST_TEST_EQ( get(m_color, u), Color::black() );
    ...
    for (i = 0; i < max_V; ++i)
      for (j = 0; j < i*i; ++j) {
        ...
        for (boost::tie(vi, vi_end) = vertices(g); vi != vi_end; ++vi)
          BOOST_TEST_EQ(get(color, *vi), Color::black());
