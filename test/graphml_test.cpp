...
int main(int, char **argv)
{
    ...
    BOOST_TEST_EQ(num_vertices(g), 9);
    BOOST_TEST_EQ(num_edges(g), 9);
    BOOST_TEST_EQ(get(vertex_color_t(), g, vertex(2,g)), 100);
    BOOST_TEST_EQ(get(vertex_color_t(), g, vertex(3,g)), 42);
    BOOST_TEST_LT(std::abs(get(edge_weight_t(), g, edge(vertex(0,g),vertex(1,g),g).first) - 0.0), 0.00001);
    BOOST_TEST_LT(std::abs(get(edge_weight_t(), g, edge(vertex(1,g),vertex(2,g),g).first) - 0.8), 0.00001);
    BOOST_TEST_EQ(get("description", dp, &g), "Root graph.");
    ...
    BOOST_TEST_EQ(num_vertices(g), num_vertices(g2));
    BOOST_TEST_EQ(num_edges(g), num_edges(g2));
    BOOST_TEST_EQ(get("description", dp, &g), get("description", dp2, &g2));

    graph_traits<graph_t>::vertex_iterator v, v_end;
    for (boost::tie(v,v_end) = vertices(g); v != v_end; ++v)
      BOOST_TEST_EQ(get(vertex_color_t(), g, *v), get(vertex_color_t(), g2, *v));

    graph_traits<graph_t>::edge_iterator e, e_end;
    for (boost::tie(e,e_end) = edges(g); e != e_end; ++e)
      BOOST_TEST_LT(std::abs(get(edge_weight_t(), g, *e) - get(edge_weight_t(), g2, *e)), 0.00001);
