...
traversal of the vertices in a directed graph. When possible, a
...
* *`vis.forward_or_cross_edge(e, g)`* is invoked on forward
or cross edges in the graph. In an undirected graph this method is never
called.
...
xref:overview/graph_theory_review.adoc#fig:dfs-example[Figure 1].

==== See Also

xref:algorithms/depth_first_visit.adoc[`depth_first_visit`]
xref:algorithms/undirected_dfs.adoc[`undirected_dfs`]
