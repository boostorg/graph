...
| The weight or "length" of each edge in the graph. The weights must all be
  ...
  xref:auxiliary/exception.adoc#negative_edge[`negative_edge`] exception if one
...
| A 4-argument function object that is passed the current value of delta_p
  ...
  *Default:*
  xref:auxiliary/layout_tolerance.adoc[`layout_tolerance`] instantiated over
...
| This parameter will be used to store the distance from every vertex to
  ...
  xref:auxiliary/BasicMatrix.adoc[`BasicMatrix`] with value type equal to the
...
* xref:algorithms/circle_layout.adoc[`circle_graph_layout`]
