...
`hawick_circuits` and `hawick_unique_circuits` accept a visitor whose `cycle(circuit, graph)` member function is called for each circuit found. The `circuit` argument is a `const`-reference to a random access sequence of `vertex_descriptor`s representing the vertices in the circuit.
