...
indicated in by the triangles and labels on the right. The user-defined
actions must be provided in the form of a visitor object, that is, an
object whose type meets the requirements for a
xref:visitors/DFSVisitor.adoc[DFS Visitor]. In the pseudo-code we show the
algorithm computing predecessors _p_, discover time _d_ and finish time
_t_. By default, the `undirected_dfs()` function does not compute
these properties, however there are pre-defined visitors such as
...
link:../../../boost/graph/undirected_dfs.hpp[`boost/graph/undirected_dfs.hpp`]
...
graph. The type `VertexColorMap` must be a model of
...
visited. The type `EdgeColorMap` must be a model of
link:../../property_map/doc/ReadWritePropertyMap.html[Read/Write
Property Map] and its key type must be the graph's edge descriptor type
and the value type of the color map must model
xref:auxiliary/ColorValue.adoc[ColorValue]. +
*Default:* none. +
graph.
____
...
* *`vis.finish_edge(e, g)`* is invoked on the back edges in the
...
An example is in
link:../example/undirected_dfs.cpp[`examples/undirected_dfs.cpp`].
