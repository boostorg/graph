...
| The topology on which the graph will be laid out. The type must model the
  xref:algorithms/topology.adoc#topology-concept[Topology] concept.
...
This algorithm [xref:about/bibliography.adoc#gursoy00[60]] performs layout of directed graphs, either weighted or unweighted. This algorithm is very different from the xref:algorithms/kamada_kawai_spring_layout.adoc[Kamada-Kawai] and xref:algorithms/fruchterman_reingold.adoc[Fruchterman-Reingold] algorithms, because it does not explicitly strive to layout graphs in a visually pleasing manner. Instead, it attempts to distribute the vertices uniformly within a _topology_ (e.g., rectangle, sphere, heart shape), keeping vertices close to their neighbors; xref:algorithms/topology.adoc[various topologies] are provided by BGL, and users can also create their own. The algorithm itself is based on http://davis.wpi.edu/~matt/courses/soms/[Self-Organizing Maps].

xref:algorithms/topology.adoc#square_topology[image:figs/ga-square.png[ga-square]]
xref:algorithms/topology.adoc#heart_topology[image:figs/ga-heart.png[ga-heart]]
xref:algorithms/topology.adoc#circle_topology[image:figs/ga-circle.png[ga-circle]]

== See Also

* xref:algorithms/kamada_kawai_spring_layout.adoc[`kamada_kawai_spring_layout`]
* xref:algorithms/fruchterman_reingold.adoc[`fruchterman_reingold_force_directed_layout`]
