...
http://graphml.graphdrawing.org/[GraphML] format. Both overloads of
[.pre]#`write_graphml`# will emit all of the properties stored in
the
link:../../property_map/doc/dynamic_property_map.html[dynamic_properties]
object, thereby retaining the properties that have been read in through
the dual function xref:io/read_graphml.adoc[read_graphml]. The second
overload must be used when the graph doesn't have an internal vertex
index map, which must then be supplied with the appropriate parameter.
...
OUT: [.pre]##`std::ostream&`##` `[.pre]#`out`#::
  A standard [.pre]#`std::ostream`# object.
IN: [.pre]##`VertexListGraph&`##` `[.pre]#`g`#::
  A directed or undirected graph. The graph's type must be a model of
  xref:concepts/VertexListGraph.adoc[VertexListGraph]. If the graph doesn't have
  an internal [.pre]#`vertex_index`# property map, one must be
  supplied with the vertex_index parameter.
IN: [.pre]##`VertexIndexMap`##` `[.pre]#`vertex_index`#::
  A vertex property map containing the indexes in the range
  [0,num_vertices(g)].
IN: [.pre]##`dynamic_properties&`##` `[.pre]#`dp`#::
  Contains all of the vertex, edge, and graph properties that should be
  emitted by the GraphML writer.
IN: [.pre]##`bool`##` `[.pre]#`ordered_vertices`#::
  This tells whether or not the order of the vertices from vertices(g)
  matches the order of the indexes. If [.pre]#`true`#, the
  [.pre]#`parse.nodeids`# graph attribute will be set to
  [.pre]#`canonical`#. Otherwise it will be set to [.pre]#`free`#.

== <<id4,Example>>

This example demonstrates using BGL-GraphML interface to write a BGL
graph into a GraphML format file.
...
against the [.pre]#`boost_graph`# library.
____
