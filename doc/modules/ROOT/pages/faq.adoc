...
|===
| Selector | Storage | Descriptors stable under remove? | Lookup / duplicates
...
...) on the graph and did not find it. Two fixes, pick one:

* Add the property as an **interior** property in the graph template
  parameters, or
* Pass the property map **explicitly** using the named-parameter API
  (`weight_map(...)`, `color_map(...)`, ...).
...
The algorithm expects a `vertex_index` property. With
`VertexList = vecS` `adjacency_list` synthesizes one; with `listS` it
does not, so you must declare it in the property chain and fill it
yourself before calling the algorithm.
