...
int main() {
    ...
    // Build the graph from the edge list. The trailing 20 is the number of
    // vertices (indices 0..19).
    ...
    // Three property maps, all keyed on a vertex:
    //   vertex_index_map : vertex -> its index   (provided by this graph type)
    //   distance         : vertex -> BFS distance from the source (wraps distance_storage)
    //   predecessor      : vertex -> its parent in the BFS tree    (wraps predecessor_storage)
