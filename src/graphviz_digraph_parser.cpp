...
/* Like YYERROR except do call bgl_dir_error.
...
do								\
  if (bgl_dir_char == YYEMPTY && yylen == 1)				\
    { bgl_dir_char = (token), bgl_dir_lval = (value);			\
      bgl_dir_char1 = YYTRANSLATE (bgl_dir_char);				\
      YYPOPSTACK;						\
      goto yybackup;						\
    }								\
  else								\
    { bgl_dir_error ("syntax error: cannot back up"); YYERROR; }	\
while (0)
...
int	bgl_dir_char;			/*  the lookahead symbol		*/
YYSTYPE	bgl_dir_lval;			/*  the semantic value of the		*/
				/*  lookahead symbol			*/
...
int bgl_dir_nerrs;			/*  number of parse errors so far       */
#endif  /* not YYPURE */

#if YYDEBUG != 0
int bgl_dir_debug;			/*  nonzero means print parse trace	*/
     ...
     YYPARSE_PARAM_DECL
{
  ...
  int bgl_dir_char1 = 0;		/*  lookahead token as an internal (translated) token number */
  ...
  if (! yyerrstatus)
      ...
      if (yyn > YYFLAG && yyn < YYLAST)
	{
	  ...
	  if (msg != 0)
	    ...
	    bgl_dir_error ("parse error; also virtual memory exceeded");
	}
      else
